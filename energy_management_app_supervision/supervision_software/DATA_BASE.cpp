...
/*bool DATA_BASE::isconnected(){
	
	return m_connexionOk;
}*/
...
void DATA_BASE::disconnect()
{
    ...
    if (m_connected) {
		...
		if (m_session) {
			delete m_session;
			m_session = nullptr; // Assurez-vous de réinitialiser le pointeur à nullptr après sa suppression
			ODBC::Connector::unregisterConnector(); // Désenregistrez le connecteur ODBC
			m_connected = false;
